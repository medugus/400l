function reportData(){
  ...
  let rates=[['ID','Name','Attended equivalents','Counted equivalents','Rate %',...courseNames],...per.map(x=>[x.id,x.name,x.a,x.n,x.rate??'',...courseNames.map(c=>x.rates[c]??'')])];
  let risk=[['ID','Name','Rate %'],...per.filter(x=>x.rate!=null&&x.rate<threshold).map(x=>[x.id,x.name,x.rate])];
  let slog=[['Session key','Date','Course','Type','Lecture #','Lecture equivalents','Block roll calls','Timetable status','Reason','Venue','Lecturer','Taken by','Marked','Present','Late','Absent'],...keys.map(k=>{let r=S.attendance[k],m=r.__meta||{},vals=Object.entries(r).filter(([x])=>x!=='__meta').map(([,v])=>v);return[k,m.date||'',m.course||'',m.type||'',m.lectureNumber||'',lectureWeight(r),m.blockRollCalls||'',m.outOfTimetable?'Out of official timetable':'Official timetable',m.reason||'',m.venue||'',m.lecturer||'',m.takenBy||'',vals.length,vals.filter(x=>x==='present').length,vals.filter(x=>x==='late').length,vals.filter(x=>x==='absent').length]})];
  let det=[['Date','Course','Type','Lecture #','Lecture equivalents','Timetable status','Reason','Venue','Lecturer','Taken by','Student ID','Student name','Status','Attended equivalents']];
  keys.forEach(k=>{let r=S.attendance[k],m=r.__meta||{};Object.entries(r).forEach(([id,status])=>{if(id==='__meta')return;let st=students.find(x=>x.id===id);det.push([m.date||'',m.course||'',m.type||'',m.lectureNumber||'',lectureWeight(r),m.outOfTimetable?'Out of official timetable':'Official timetable',m.reason||'',m.venue||'',m.lecturer||'',m.takenBy||'',id,st?.name||'',markLabel(status),(status==='present'||status==='late')?lectureWeight(r):0])})});
